...
去AI味不是把文字全部磨成句号。质问里的 `？`、爆发峰值的少量 `！`、犹豫/未尽的 `……`、对话打断/拖长的 `——`，只要承载语气和人物声线就应保留或重排；需要清理的是叙述破折号、无功能的 `!!!`、省略号刷屏和随机标点堆砌。
...
- 偶尔用不完整句（口语感）
- 段落长短交错（不要每段都3-5行）
- 标点节奏跟语气走：避免通篇句号化；保留有功能的 `？` / `！` / `……` / 对话 `——`，删除随机堆砌或刷屏符号
...
处理方法：
- 加入口语化表达（"嗯""哦""行吧"）
- 适当打断对话（角色可以答非所问）；对话被打断或拖长时可保留 `——`
- 用动作穿插对话（"她喝了口水。'然后呢？'"）
- 删掉解释性对话（角色不会把自己的动机说清楚）
- 不把所有对话末尾改成句号：质问保留问号，爆发峰值保留少量感叹，吞回去/没说完可用 `……` 或动作停顿
...
作用边界：
- 只处理正文里的叙述破折号 `——`、`—`、双连字符 `--` 和独立行 `---`；默认不改变引号风格，也不把 `？` / `！` / `……` 改成句号。
- 知乎盐言短篇可保留 `「」`；只有用户或项目明确要求时，才加 `--quote-mode ascii` 或 `--quote-mode yan`。
- 对话中表示被打断或拖长的 `——` 是合法语气标点，脚本会保留；无功能标点堆砌由人工 Gate D/E 判断处理。
...
- 节奏调整：{N} 处
- 对话优化：{N} 处
- 标点节奏调整：{N} 处（保留有功能 `？`/`！`/`……`/对话 `——`，清理无功能堆砌或叙述破折号）
- 结尾修正：{N} 处
