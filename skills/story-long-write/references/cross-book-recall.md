...
## 触发
项目根 `拆文库/` 或项目 `对标/` ≥2 本启用。主对标书取 `设定/题材定位.md`「主对标书」字段，缺失则用字典序第一本并提示用户补。

> **数量规则**：主对标书最多 1 本，用于文风和最终正文输入；副对标 / 参考对标**不限制登记数量**。执行时不是只取 1 本副书，而是按题材相关性、引用强度和阶段预算逐本召回，超过预算时裁剪条目，不删除书目。

## 三道防线
1. 副对标 `文风.md` 不读，避免多本文风互相污染
2. 角色/剧情/设定模块可从所有副对标召回，但必须按「同题材 > 弱相关 > 参考」排序，并受每本/总量预算约束
3. narrative-writer 正文 prompt 只吃主对标文风与最终筛选后的摘要，不把多本副书原文直接塞入正文生成
...
排序规则：先按相关性（同题材 > 弱相关），再按引用强度（辅 > 参考），再按用户在 `对标书列表` 中的顺序。副书数量不限；如果总条数超过阶段预算，裁条目不裁书目记录。
...
|---|---|---|---|---|---|
| {书名} | 辅/参考 | 同题材/弱相关 | 设定/大纲/模块 | {n} | {用于补充某类结构，不进入文风} |
```

若副书很多，只输出被本阶段实际召回的条目；未召回的副书不代表被删除，只是本阶段预算未命中。
