...
5. **蓝图兑现检查**：新版细纲存在时，核对正文是否消费了内容概括五段式、情节安排多线、人物关系变化/出场顺序、代价兑现/收益兑现；旧版细纲只核对核心事件、目标情绪、章首/章尾钩子和字数目标
...
7. **标点确定性收尾**：对本批所有新写正文文件运行 `node scripts/normalize-punctuation.js 正文/第XXX章_*.md`（写模式，默认 `--quote-mode keep`），确定性去掉正文里的 `……`、破折号 `——`/`—`、双连字符 `--` 和独立行 `---`，避免长篇写到上百章后横线或长省略累积。脚本不再保留对话打断破折号；数字区间会改写为中文连接词，盐言「」不受影响。**由谁运行**：narrative-writer agent 不运行本脚本；主会话在 agent 写完返回后、针对实际落盘的正文文件路径运行（文件名是写完才确定的 `第XXX章_章名.md`）。
