...
**标点确定性收尾**：本批正文写完后，对所有新写正文文件运行 `node scripts/normalize-punctuation.js 正文/第XXX章_*.md`（写模式，默认 `--quote-mode keep`），确定性清除正文里的 `……`、破折号 `——`/`—`、双连字符 `--` 和独立行 `---`，防止长篇累积横线和长省略。对话打断、拖长音与数字区间不设破折号例外；盐言「」引号不受影响。narrative-writer agent 不运行本脚本，由主会话在 agent 返回后针对实际落盘文件运行。
