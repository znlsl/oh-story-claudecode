...
排名 | 书名(需详情页解码) | 作者(需详情页解码) | 题材(详情页) | 状态 | 在读(核心指标) | 字数 | 评分(详情页) | 标签(详情页) | 最新更新 | bookId | 作品页链接 | 简介(截断100字)
...
> 标题/作者/题材/评分/标签/简介均为可选字段：详情页拿到才输出。书名解码失败时书名显示 `（标题待解析）`，但 bookId 与作品页链接始终保留，便于人工回查。

### 采集要点

字体反爬：列表页 innerText 被自定义字体混淆，`scripts/fanqie-rank-scraper.js` 改从详情页 HTML（内嵌 JSON + `<title>` + og:meta）多策略解码明文，规避字体反爬。流程：访问品类页 → 提取品类链接 → 逐品类取 `__INITIAL_STATE__` 列表 → 分批（每 5 本）请求详情页解码。单页上限约 20 本需滚动加载；`--top N` 可调每题材上限。
...
排名 | 书名 | 作者 | 题材/频道 | 字数 | 收藏数(核心) | 营养液 | 积分 | 状态 | 链接 | 简介(截断100字)

### 采集要点

榜单页结构老旧排版密集需精确定位。收藏数/营养液/积分是核心指标。部分需登录态。
