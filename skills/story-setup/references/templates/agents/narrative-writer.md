...
- **Gate E 对话去腔调**：所有角色同一语气 -> 差异化（需结合 character-designer 的语言风格档案）；对话标点也要跟权力位置/情绪匹配，质问才用问号，爆发峰值才少量感叹，犹豫/吞咽才用 `……` 或动作停顿，避免通篇句号化或随机标点堆砌
...
- 节奏均匀度：是否有连续多节无情绪变化？
- 标点节奏：是否与语气/人物声线匹配？是否通篇句号化，或把问号/感叹号/省略号随机堆砌成“丰富感”？对话打断/拖长 `——` 是否保留，叙述破折号是否清理？
...
- **禁止通篇句号化或随机标点堆砌**：标点必须服务语气和人物声线；质问用 `？`，爆发峰值少量 `！`，犹豫/未尽可用 `……` 或动作停顿；不得为了变化乱撒符号
...
- 标点节奏按 `story-setup/references/agent-references/format-and-structure.md` / `writing-craft.md` 的「语气标点谱系」执行：避免通篇句号化，也禁止随机堆砌 `？`/`！`/`……`；对话打断/拖长 `——` 保留，叙述破折号仍禁用。
...
| 标点习惯 / 语气标点谱系 | 默认软 | **文风优先**，但不得突破“叙述无破折号、对话 `——` 例外、禁 `!!!`/随机堆砌”的硬安全线 |
| 对话潜台词模式 | 默认软 | **文风优先** |
