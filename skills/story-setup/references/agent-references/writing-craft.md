...
三维度织入之前，先定叙述姿态。默认锁死主视角角色的"此刻感知"——只写他此刻看到、听到、闻到、身体感到、脑中闪过的东西。这一条同时解决两个最顽固的 AI 味：代入感不足，和"作者在场"的说教/上帝腔。
...
标点不是装饰，而是语气、权力关系和情绪节拍的可见形态。写正文时先判断“这句话在场景里做什么”，再决定用句号、逗号、问号、感叹号、省略号、冒号或对话破折号。
...
| 犹豫 / 未说完 / 心虚 | `……`、逗号、吞咽/停手等动作 | 省略号只放在真的说不完、吞回去、拖住气口的位置 | 不用 `……` 替代所有停顿 |
| 被打断 / 拖长音 | 对话内 `——` | 只用于引号里的打断或拖长，如 `"我没——"` | 叙述正文不用 `——`/`—`/`--` |
| 信息揭示 / 判断落点 | 冒号、分号、短句 | “证据只有一个：……”可用冒号制造落点 | 不写论文式长分号链 |

### 执行步骤

1. 读本场景目标情绪和角色声线，给每个关键对话 beat 标一类语气（压迫/试探/爆发/犹豫/打断）。
2. 精修时扫一遍句尾：如果一整段只剩句号，检查是否把质问、迟疑、爆点压平；如果满屏 `？`/`！`/`……`，删到只保留有功能的位置。
3. 对话里的 `——` 可保留；叙述里的破折号必须改为句号/逗号/动作断句。知乎盐言 `「」` 只是引号风格，不影响问号、感叹号、省略号的合法使用。
