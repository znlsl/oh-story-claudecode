...
3. **检查核心 Agent 部署状态**（只检查项目内 agents，不要假设一定存在）：
   ...
   - 如果 `.story-deployed` 存在且 `agents_version` 缺失或小于 `13`，视为 stale deployment；不要 spawn，降级 `solo`，建议用户重新运行 `/story-setup`。
...
- 标点节奏：标点是否服务语气/人物声线；通篇句号化或随机堆砌问号、感叹号、省略号按影响定 S3/S2。
...
6. **确定性标点预检（只报告，不修改）**：当审查范围包含本地正文文件路径时，运行本 skill 自带脚本：
   ...
   ```
   - 将 `em-dash`、`double-hyphen`、`markdown-divider` 结果作为 `format` 或 `prose` findings 合并进报告；另外人工检查标点节奏是否通篇句号化或随机堆砌，脚本不替代语气判断。
...
- 提示指令：
  ...
  4. 标点节奏是否匹配语气/人物声线：是否通篇句号化，或随机堆砌问号/感叹号/省略号？对话打断/拖长 `——` 是否被保留、叙述破折号是否清理？
