...
> **语料读取**：本阶段及阶段 4a/4c、孤立情节兜底里的「全部章节摘要 / 全部情节点」统一从 `_章节摘要汇总.md`（Stage 2 收尾按章号无损拼接的全书摘要，见 SKILL.md「Stage 2 收尾：合并章节摘要」）**一次性读取、在上下文中复用**，不逐文件 glob 重扫——同一份语料的 4-5 次冷读降为 1 次。汇总文件缺失或语料超上下文时，回退原「扫描全部章节摘要」逐文件方式 / 分块策略，行为不变。
...
2. **`剧情/情绪模块.md`（模块权威）**
   ...
   - 生成法：先抽象“读者想看什么”，再把具体剧情降维为情绪链和功能位；模块卡必须包含可替换项和不可照搬项。
   - 校验：每个模块至少关联一个 `剧情/节奏.md` 的 RH/TR 条目；禁止把原文专名、独特事件顺序、标志性台词写成可复用模板。
...
| 模块可追溯 | 100% | 情绪模块卡关联到 RH/TR 或章节情节点的比例 | 无来源模块删除或补证据 |
