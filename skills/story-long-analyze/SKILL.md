...
### Stage 2 收尾：合并情节点台账（_台账.md）

Stage 2 所有 `章节/*_摘要.md` 落盘后、进入 Stage 3 前，主线程把它们按章号顺序**无损拼接**成 `拆文库/{书名}/_台账.md`（只拼接、不压缩、不改写）：

```bash
ls 章节/*_摘要.md | sed -E 's/.*第([0-9]+)章.*/\1 &/' | sort -n | cut -d' ' -f2- | while read -r f; do cat "$f"; echo; done > _台账.md
```

**无损门控**（拼接后校验，任一不过即删除 `_台账.md`、回退逐文件扫描，行为不变）：
- `grep -cE '^P[0-9]+ ' _台账.md` == 各摘要 `^P` 行数之和
- `grep -cE '^## 第' _台账.md` == 摘要文件数

Stage 3 / 4a / 4c / 孤立情节兜底改为**只读一次 `_台账.md`** 并在上下文中复用，替代每阶段 `glob 章节/*_摘要.md` 重扫（同一份语料的 4-5 次冷读降为 1 次）。

**仅当语料能放进上下文时建台账**：>500 章、或合并后 `_台账.md` 过大放不进上下文时**不建台账**，走下方「分块策略」。`_台账.md` 不替代 `章节/*_摘要.md`——单章文件仍是落盘真源，Stage 6 文风采样、人工复核照用单章文件。
